#AUTHORS : BATIKAN ÖZKUL, HAKAN ACUNDAŞ, YUNUS EMRE SANCAK
...
pygame.display.set_caption("Colloseum")
...
class Gladiator(pygame.sprite.Sprite):
    def __init__(self, x, y, NN=None):
        ...
        self.GLAD_SPRITE = pygame.image.load("Glad.PNG").convert_alpha()
        self.image = pygame.transform.scale(GLAD_SPRITE, (width, heigth))
        self.x = x
        self.y = y
        self.a = 0
        self.v = 0
    ...
    def rotation(self, rotateval):
        ...
        if rotateval < -0.05:
            self.angle += angle_rate
        self.rect.center = (self.x, self.y)
        self.image = pygame.transform.rotate(GLAD_SPRITE, self.angle)
...
def gameLoop(g1, g2, fps=10000000000000000):
    ...
    while not gameExit:
        ...
        message_to_screen(str((match_cooldown - gameTimer) / FPS), red, 10)
        g1.draw_Glad()
        g2.draw_Glad()
        g1.decide(inputgec)
        g2.decide(inputgec2)
